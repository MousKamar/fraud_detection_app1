...
app.post('/upload', upload.single('file'),async (req, res) => {
    ...
    if (req.file) {
       ...
       try {
            const response = await axios.post('http://fraud_detection_app-backend-1:5010/upload', form, {
...
app.post('/upload_train_file',upload.single('train_file'), async (req, res) => {
    ...
    if (req.file) {
        ...
        form.append('file', fs.createReadStream(filePath), fileName);
       try {
            const response = await axios.post('http://fraud_detection_app-backend-1:5010/upload_train_file', form, {
                headers: form.getHeaders()
            });
...
app.listen(port, () => {
    console.log(`Frontend server running at http://fraud_detection_app-backend-1:${port}`);
});
